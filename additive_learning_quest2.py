...
def load_dataset():
    ...
    for i in range(10):
        temp = np.array(train_data[train_data[:, 0] == i])
        m, n = temp.shape
        temp = temp[0:m].T
        label_wise_dataset["X_train_" + str(i)] = temp[1:n] / 255
        label_wise_dataset["Y_train_" + str(i)] = temp[0].astype(int)
    m, n = train_data.shape
    train_data = train_data[0:m].T
    label_wise_dataset["X_train"] = train_data[1:n] / 255
    label_wise_dataset["Y_train"] = train_data[0].astype(int)
    keys = label_wise_dataset.keys()
    return label_wise_dataset
...
def cat_cross_entropy(one_hot_Y, a4):
    ...
    CCE = -np.mean(one_hot_Y*np.log(a4))
    return CCE
...
# Model: [784, 256, 128, 32, 10]
def gradient_descent_additive(model_params, label_wise_dataset, learning_rate, n_epochs, mini_batch_size):
    loss_values = []
    accuracy_values = []
    X_train = label_wise_dataset["X_train"]
    Y_train = label_wise_dataset["Y_train"]
    for i in range(n_epochs):
    	num = random.randint(0, 9)
    	X = label_wise_dataset["X_train_" + str(num)]
    	Y = label_wise_dataset["Y_train_" + str(num)]
    	indices = np.random.randint(0, Y.size, mini_batch_size)
    	X = X[:, indices]
    	Y = Y[indices]
    	...
    	interim_values["z3"] = model_params["w3"].dot(interim_values["a2"]) + model_params["b3"]
    	interim_values["a3"] = relu(interim_values["z3"])
    	n_a3_nodes, n_records = interim_values["a3"].shape
    	collapsed_a3 = np.array(interim_values["a3"].mean(axis=1).reshape(n_a3_nodes, -1))
    	# interim_values["z4"] = model_params["w4"].dot(interim_values["a3"]) + model_params["b4"]
    	interim_values["z4"] = model_params["w4"].dot(collapsed_a3) + model_params["b4"]
    	# interim_values["a4"] = softmax(interim_values["z4"])
    	interim_values["a4"] = np.repeat(softmax(interim_values["z4"]), repeats=n_records, axis=1).reshape(10, -1)
    	...
    	if((i + 1) % 1 == 0):
    	    loss = cat_cross_entropy(one_hot(Y), interim_values["a4"])
    	    model_output = forward(model_params, X_train)
    	    model_output = model_output["a4"]
    	    # print(model_output.shape)
    	    predictions = get_predictions(model_output)
    	    # print(predictions[:10])
    	    acc = accuracy(predictions, Y_train) * 100
    	    loss_values.append(loss)
    	    accuracy_values.append(acc)
    	    print(f"Minibatch: {i + 1}\tAccuracy: {acc:f}%\tLoss: {loss}")
    return model_params
...
n_epochs = 1000
mini_batch_size = 32
label_wise_dataset = load_dataset()
model_params = load_model(filename)
gradient_descent_additive(model_params, label_wise_dataset, learning_rate, n_epochs, mini_batch_size)
